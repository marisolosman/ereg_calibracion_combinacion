...
for l in `seq -f "%02g" 1 12`; do 
	for k in {1982..2010} ;	do #loop sobre anios
		for j in "${variables[@]}" ; do
			...
			for m in {1..28} ; do
				if [ $m -le 4 ] ; then
					...
					if [ ! -f "$FILE" ] ; then
						wget -O "$FILE" "${ruta_iri}.NASA-GEOSS2S/.HINDCAST/.MONTHLY/.${j}/S/%280000%201%20${MONTHS[${l#0}]}%20${k}%20%29VALUES/M/%28${m}.0%20%29VALUES/data.nc"
					fi
				fi
				if [ $m -le 10 ] ; then
				...
				if [ $m -gt 24 ] && [ ${l} -eq 11 ] ; then
					...
					if [ ! -f "$FILE" ] ; then
						wget -O "$FILE" "${ruta_iri}.NCEP-CFSv2/.HINDCAST/.PENTAD_SAMPLES/.MONTHLY/.${j}/S/%280000%201%20${MONTHS[${l#0}]}%20${k}%29VALUES/M/%28${m}.0%20%29VALUES/data.nc"
					fi 
				else
					FILE=${ruta}${j}_Amon_NCEP-CFSv2_${k}${l}_r${m}_${k}${l}-${FY}${mespr}.nc
					if [ ! -f "$FILE" ] ; then
				...
				find ${ruta}${j}_*_${k}*.nc -size 0M -delete
