...
def main():
    ...
    parser.add_argument('CV', type=bool, nargs=1,\
            help='Croos-validations mode (True or False)')
    ...
    if archivo.is_file():
        ...
        if args.CV[0]:
            terciles = obs.computo_terciles(obs_dt, args.CV[0]) # Obtain tercile limits
    ...
    if np.logical_not(args.CV[0]):
        ...
        if archivo2.is_file():
            ...
            [lats_obs, lons_obs, obs_3m] = obs.select_months(calendar.month_abbr[\
                                                             ...
                                                             coords['lat_n'], coords['lon_w'],
                                                             coords['lon_e'])
            obs_dt = obs.remove_trend(obs_3m, args.CV[0]) #Standardize and detrend observation
            terciles = obs.computo_terciles(obs_dt, args.CV[0]) # Obtain tercile limits
        ...
        if np.logical_not(args.CV[0]):
            ...
            if output2.is_file():
                ...
                [lats, lons, pronos] = modelo.select_months(args.IC[0], \
                                                            ...
                                                            coords['lon_w'],
                                                            coords['lon_e'])
                [pronos_dt, a1, b1] = modelo.remove_trend(pronos, args.CV[0])
