...
mean_index_array = np.zeros(7*7*12, dtype = {'names': index, 
    'formats': ['f8', 'f8', 'f8', 'f8', 'f8']})
mean_index_array_trop_SA = np.zeros(7*7*12, dtype = {'names': index, 
    'formats': ['f8', 'f8', 'f8', 'f8', 'f8']})
mean_index_array_extratrop_SA = np.zeros(7*7*12, dtype = {'names': index, 
    'formats': ['f8', 'f8', 'f8', 'f8', 'f8']})
...
for ii in index:
    ...
    for leadtime in np.arange(7):
        ...
        plt.title('Mean '+ ii.upper() + ' - Leadtime '+ str(leadtime + 1) + ' month - ALL')
        plt.savefig(route + ii + '_leadtime_' + str(leadtime + 1) + '_month_all.png',
...
for ii in index:
    ...
    for i in np.arange(7):
        ...
        plt.figure()
        if (ii == 'auroc_above') or (ii == 'auroc_below'):
            plt.pcolor(np.transpose(aux[i,:,:]),edgecolors='k', linewidth=2,cmap=cmap, vmin = -0.6, vmax = 0.6)
        else:
            plt.pcolor(np.transpose(aux[i,:,:]),edgecolors='k', linewidth=2,cmap=cmap, vmin = -0.3, vmax = 0.3)

        plt.xlabel ('Target Season')
        plt.ylabel ('Leadtime')
        plt.xticks(np.arange(0.5,12.5,1), ('JFM','FMA','MAM','AMJ','MJJ','JJA','JAS',\
                'ASO','SON','OND','NDJ','DJF'),rotation = 20)
        plt.yticks(np.arange(0.5,7.5,1), ('1','2','3','4','5','6','7'))
        plt.colorbar()
        plt.title('Mean '+ ii.upper() + ' -  '+ leyenda[i] +' - ALL')
        plt.savefig(route + ii + '_all_' + leyenda[i] + '.png', dpi = 300, bbox_inches = 'tight',\
                papertype = 'A4')
        plt.close()
for ii in index:
    aux = np.reshape(mean_index_array_trop_SA[ii],[7,12,7])
    for i in np.arange(7):
        ...
        plt.savefig(route + ii + '_trop_SA_' + leyenda[i] + '.png', dpi = 300, bbox_inches = 'tight',\
                papertype = 'A4')
        plt.close()
...
for ii in index:
    ...
    for i in np.arange(7):
        ...
        plt.yticks(np.arange(0.5,7.5,1), ('1','2','3','4','5','6','7'))
        plt.colorbar()
        plt.title('Mean '+ ii.upper() + ' -  '+ leyenda[i] + ' - Extratropical SA')
        plt.savefig(route + ii + '_extratrop_SA_' + leyenda[i] + '.png', dpi = 300, bbox_inches = 'tight',\
                papertype = 'A4')
        plt.close()

#plot mean score vs lead for all cal-comb method
route = '/datos/osman/nmme_figuras/synthesis_figures/scores_vs_leadtime/'
seas = ['JFM','FMA','MAM','AMJ','MJJ','JJA','JAS',\
        'ASO','SON','OND','NDJ','DJF']
for ii in index:
    aux = np.reshape(mean_index_array[ii],[7,12,7])
...
for ii in index:
    ...
    for season in np.arange(12):
        ...
        plt.legend(bbox_to_anchor = (1.04,0.5), loc = "center left")
        plt.title('Mean '+ ii.upper() + ' - Target: '+ seas[season] + ' - Trop SA')
...
for ii in index:
    ...
    for season in np.arange(12):
        ...
        plt.legend(bbox_to_anchor = (1.04,0.5), loc = "center left")
        plt.title('Mean '+ ii.upper() + ' - Target: '+ seas[season] + ' - Extratrop SA')
