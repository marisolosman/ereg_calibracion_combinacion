...
def manipular_nc(archivo, variable, lat_name, lon_name, lats, latn, lonw, lone, last_month, year_init):
    ...
    #como el resampling trimestral toma el ultimo mes como parametro, asi genero el promedio trimestral
    ...
    var_out = var_out.sel(time = np.logical_and(var_out['time.month'] == mes,
        np.logical_and(var_out['time.year'] >= year_init,var_out['time.year'] <= (year_init+hind_length))))
    return var_out, lat, lon

class Observ(object):
    def __init__(self, institution, var_name, lat_name, lon_name, date_begin, date_end):
...
#comienzo la definición de los métodos
    ...
    def select_months(self, last_month, year_init, lats, latn, lonw, lone): 
        ...
        [variable, latitudes, longitudes] = manipular_nc (file, self.var_name, self.lat_name,
                self.lon_name, lats, latn, lonw, lone, last_month, year_init)

        #tengo que convertir el dato de lluvia al mismo tipo que el del modelo (mm/day) con
        #30-day month type
    ...
    def remove_trend(self, observation, CV_opt):  
        ...
        #obs 3-D array array ntimes nlats nlons CV_opt boolean
